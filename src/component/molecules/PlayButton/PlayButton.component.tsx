...
export function PlayIcon(props: PlayButtonIconProps) {
  const { enabled = true } = props;
  return (
    <svg
      style={{ fill: enabled ? '#007bff' : '#dadada' }}
...
export function PauseIcon(props: PlayButtonIconProps) {
  const { enabled = true } = props;
  return (
    <svg
      style={{ fill: enabled ? '#007bff' : '#dadada' }}
...
function PlayButton(props: PlayButtonProps) {
  ...
  return (
    ...
    >
      {isPlaying ? <PauseIcon enabled={true} /> : <PlayIcon enabled={true} />}
    </div>
  );
}
