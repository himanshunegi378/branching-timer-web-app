  ...
  constructor(/* list of at least event names and one optional target object (in any order) */) {
    ...
    for (var i = 0; i < arguments.length; i++) {
      ...
      switch (typeof arg) {
        ...
        default:
          throw new TypeError(
            'Eventjs() only accepts string and object parameters'
          );
      }
    }
    ...
    if (numEventNames === 0) {
      throw new Error(
        'There should be at least one event name (string) for the Eventjs to be useful.'
      );
    }

    if (target === this && !(this instanceof Eventjs)) {
      throw new ReferenceError(
        'Eventjs is not called with "new" keyword and no parameter of type object is passed to it'
      );
    }
    ...
    function checkEventName(eventName) {
      if (typeof eventName !== 'string' || !events[eventName]) {
        throw new ReferenceError(
          'The event name does not exist in this event manager: ' + eventName
        );
      }
      return true;
    }
    ...
    target.trigger = function (
      eventName /*, optional list of arguments to be passed to event handler */
    ) {
      checkEventName(eventName);
      var argumentList = [];
      ...
      if (errors.length > 0) {
        var newError = new Error(
          'At least one of the event listeners was interrupted with an error. See errors'
        );
        console.log(errors);
